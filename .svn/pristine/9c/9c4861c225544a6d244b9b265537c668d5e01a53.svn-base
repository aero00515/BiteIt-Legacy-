...
import edu.utboy.biteit.asynctasks.VerifyUserRequest;
...
public class TutorialActivity extends FragmentActivity {
	...
	private static final int VERIFY_CONFIRM = 0;
		...
		public void onNext(String stdId) {
			if (newUserData != null) {
				if (stdId != null && stdId.matches("[ug]?[0-9]{7,8}")) {
					...
					registerInBackground(newUserData);
			...
			if (loginType.equals("facebook")) {
				if (Session.getActiveSession().isOpened()) {
					...
					if (userId != null && !userId.isEmpty()) {
						...
						// TODO verify user is exists or not
	...
	private Response.Listener<JSONObject> verifyUserResponseListener = new Response.Listener<JSONObject>() {
			...
			new Thread() {
				...
				public void run() {
					...
					try {
						...
						MyLog.d(TutorialActivity.class,
								"Device registered, registration ID=" + regid);

						// TODO update regID to server
						MyLog.d(TutorialActivity.class,
								"" + response.toString());
						boolean isVerified = response.getBoolean("is_user");
						if (isVerified) {
							JSONObject dataToSync = response
									.getJSONObject("data");
							JSONObject userJson = dataToSync
									.getJSONObject("user");
							JSONObject userDetailJson = dataToSync
									.getJSONObject("user_detail");
							Bundle data = new Bundle();
							data.putString(
									SyncUserDataService.SHARED_PREFERENCED_BASE,
									"");
							data.putString(
									SyncUserDataService.SHARED_PREFERENCED_TOKEN,
									dataToSync.getString("token"));
							data.putInt(
									SyncUserDataService.SHARED_PREFERENCED_ID,
									userJson.getInt("id"));
							data.putString(
									SyncUserDataService.SHARED_PREFERENCED_USERNAME,
									userJson.getString("username"));
							data.putString(
									SyncUserDataService.SHARED_PREFERENCED_NAME,
									userJson.getString("first_name"));
							data.putString(
									SyncUserDataService.SHARED_PREFERENCED_EMAIL,
									userJson.getString("email"));
							data.putString(
									SyncUserDataService.SHARED_PREFERENCED_LOGIN_TYPE,
									userDetailJson.getString("login_type"));
							data.putString(
									SyncUserDataService.SHARED_PREFERENCED_THIRD_ID,
									userDetailJson.getString("third_id"));
							data.putString(
									SyncUserDataService.SHARED_PREFERENCED_GCM_ID,
									userDetailJson.getString("gcm_id"));
							data.putString("friends",
									dataToSync.getString("friends"));
							data.putString("feedbackpoint",
									dataToSync.getString("feedbackpoint"));
							Intent intent = new Intent();
							...
							verifyHandler.dataHandler
									.sendEmptyMessage(VERIFY_CONFIRM);
						}
	...
	private void verifyUser(Bundle newUserData) {
		VolleyTaskManager.getInstance().init(getAssets());
		try {
			JSONObject requestBody = VerifyUserRequest.getInstance()
					.getVerifyUserJson(newUserData);
			VerifyUserRequest.getInstance().verifyUser(getApplicationContext(),
					requestBody, verifyUserResponseListener,
					new Response.ErrorListener() {

						@Override
						public void onErrorResponse(VolleyError error) {
							MyLog.e(TutorialActivity.class,
									"" + error.toString());
						}
					});
		} catch (JSONException e) {
		...
		new Thread() {
			...
			public void run() {
				try {
					...
					MyLog.d(TutorialActivity.class, "Error :" + ex.getMessage());
					...
					new Response.Listener<JSONObject>() {
						...
						public void onResponse(JSONObject response) {
							...
							try {
								...
								data.putString("friends",
										response.getString("friends"));
								data.putInt("id",
										response.getJSONObject("user_info")
												.getInt("id"));
								// TODO token hasn't come back
							} catch (JSONException e) {
								e.printStackTrace();
							}
							Intent intent = new Intent();
	...
	private HandlerCallBack verifyHandler = new HandlerCallBack() {

		@Override
		public void handleDataMessage(Message msg) {
			if (msg.what == VERIFY_CONFIRM) {
				showNextFragment();
			}
		}
	};
